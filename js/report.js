class ReportManager {
    ...
    async saveReport() {
        ...
        try {
            ...
            const existingReports = JSON.parse(localStorage.getItem('civicconnect_reports') || '[]');
            existingReports.push(formData);
            localStorage.setItem('civicconnect_reports', JSON.stringify(existingReports));
        }
        ...
        Array.from(files).slice(0, 5).forEach(file => {
            if (file.type.startsWith('image/') && file.size <= 5 * 1024 * 1024) {
                ...
                reader.onload = (e) => {
                    ...
                    this.addPhotoToPreview(e.target.result);
                    // Reattach click listener after adding images
                    this.attachPhotoPreviewListener();
                };
